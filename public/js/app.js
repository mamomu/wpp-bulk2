...
async function loadMessageHistory() {
  try {
    ...
    if (!data.success) {
      throw new Error(data.error || "Falha ao carregar histórico de mensagens");
    }

    if (data.history.length === 0) {
      historyContent.innerHTML =
        '<p class="text-gray-500 text-center">Nenhum histórico de mensagens encontrado</p>';
      return;
    }

    const historyHTML = data.history
      .map((item) => {
        const successDetails = item.report.details.filter(
          (d) => d.status === "success"
        );
        const failedDetails = item.report.details.filter(
          (d) => d.status === "failed"
        );
        ...
        <div class="history-item">
            <div class="history-item-header">
                <span class="history-timestamp">${formatDate(
                  item.timestamp
                )}</span>
                <button 
                    type="button"
            ...
            <div class="history-stats">
                <div class="stat-item total cursor-pointer" 
                     onclick='showContactDetails(${JSON.stringify(
                       totalDetails
                     ).replace(/'/g, "&apos;")}, "Total")'>
...
async function checkStatus() {
  try {
    ...
    statusValue.textContent = data.isClientReady ? "Sim" : "Não";
    statusValue.className = data.isClientReady
    ...
    if (data.isClientReady) {
      ...
      document.getElementById(
        "qrCode"
      ).innerHTML = `<img src="${data.qrCode}" alt="QR Code" class="max-w-xs">`;
...
document.getElementById("messageForm").addEventListener("submit", async (e) => {
  ...
  formData.append(
    "contactFile",
    document.getElementById("contactFile").files[0]
  );
            ...
            <div class="space-y-4">
                <div class="grid grid-cols-3 gap-4">
                    <div class="bg-green-50 p-4 rounded-lg">
                        <p class="text-lg font-semibold text-green-700">Total: ${
                          sendData.report.total
                        }</p>
                    </div>
                    <div class="bg-blue-50 p-4 rounded-lg">
                        <p class="text-lg font-semibold text-blue-700">Sucesso: ${
                          sendData.report.successful
                        }</p>
                    </div>
                    <div class="bg-red-50 p-4 rounded-lg">
                        <p class="text-lg font-semibold text-red-700">Falha: ${
                          sendData.report.failed
                        }</p>
                    </div>
                </div>
                <div class="mt-6">
                    <h3 class="text-lg font-semibold mb-3">Relatório Detalhado</h3>
                    <ul class="space-y-2">
                        ${sendData.report.details
                          .map(
                            (detail) => `
                            <li class="p-3 rounded-lg ${
                              ...
                              detail.status === "success" ? "sucesso" : "falha"
                            }
                                ${
                                  detail.error
                                    ? `<br><span class="text-sm opacity-75">Erro: ${detail.error}</span>`
                                    : ""
                                }
                            </li>
                        `
...
document.getElementById("logoutBtn").addEventListener("click", async () => {
  if (!confirm("Tem certeza que deseja sair do WhatsApp?")) {
    return;
  }
...
socket.on("qr", (qr) => {
  document.getElementById(
    "qrCode"
  ).innerHTML = `<img src="${qr}" alt="QR Code" class="max-w-xs">`;
  document.getElementById("loadingText").classList.add("hidden");
});
...
socket.on("qrDataUrl", (qrDataUrl) => {
  document.getElementById(
    "qrCode"
  ).innerHTML = `<img src="${qrDataUrl}" alt="QR Code" class="max-w-xs">`;
  document.getElementById("loadingText").classList.add("hidden");
});
