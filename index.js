...
const upload = multer({
  ...
  fileFilter: (req, file, cb) => {
    ...
    if ([".xlsx", ".xls", ".png", ".jpg", ".jpeg"].includes(ext)) {
      ...
      cb(
        new Error("Invalid file type. Only Excel and image files are allowed.")
      );
    }
  },
});
...
const initializeClient = async (retries = 3, delay = 5000) => {
  for (let i = 0; i < retries; i++) {
    try {
      console.log(
        `Initializing WhatsApp client (attempt ${i + 1}/${retries})...`
      );
      await client.initialize();
      return;
    } catch (error) {
      console.error(
        `Failed to initialize WhatsApp client (attempt ${i + 1}):`,
        error
      );
      if (i < retries - 1) {
  ...
  throw new Error(
    "Failed to initialize WhatsApp client after multiple attempts"
  );
};
...
app.post(
  ...
  async (req, res) => {
    try {
      ...
      const messageFileName = await saveMessage(
        req.files["contactFile"][0].filename,
        req.body.message,
        contacts,
        initialReport
      );
      ...
      contacts.map(async (contact) => {
        try {
          const imagePath = imageFile
            ? path.join(CONFIG.uploadDir, imageFile)
            : null;
        ...
        await new Promise((resolve) =>
          setTimeout(resolve, CONFIG.messageDelay)
        );
      })
    );
